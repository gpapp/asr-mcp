...
class VoiceprintDB:
    ...
    def save(self, name: str, embedding: np.ndarray, pitch_hz: float = 0.0,
             pitch_std: float = 0.0, energy_rms: float = 0.0,
        ...
        with self._db.get_session() as session:
            existing = session.get(VoiceprintModel, name)
    ...
    def get(self, name: str) -> Optional[dict]:
        with self._db.get_session() as session:
            vp = session.get(VoiceprintModel, name)
            if not vp:
                return None
    ...
    def list_all(self) -> dict[str, dict]:
        with self._db.get_session() as session:
            rows = session.query(VoiceprintModel).all()
